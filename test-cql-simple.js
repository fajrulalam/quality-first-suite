...
mcpServer.stdout.on('data', (data) => {
  try {
    ...
    if (response.id === 1 && !searchComplete) {
...
`;
        ...
        logContent += 'No pages found with this query.\n\n=== End of Log ===';
        fs.writeFileSync(logFileName, logContent);
        console.log(`\nTest completed! Results saved to: ${logFileName}`);
        mcpServer.kill();
        process.exit(0);
      }
    } else if (response.id === 2 && !pageContentComplete) {
      // This is the page content response
      pageContentComplete = true;
...
// Cleanup after 20 seconds if no response
setTimeout(() => {
  if (!pageContentComplete) {
    logContent += '\n\n=== Test Timed Out ===';
    fs.writeFileSync(logFileName, logContent);
