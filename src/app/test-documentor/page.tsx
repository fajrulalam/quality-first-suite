...
function TestDocumentorContent() {
  ...
  useEffect(() => {
    if (!runId) {
      // If no runId, redirect to test runs page
    ...
    console.log(`Loading test documentor data for ${runId}:`, savedData);
    if (savedData) {
      try {
        const parsedData: LocalStorageData = JSON.parse(savedData);
        console.log("Parsed test cases:", parsedData.testCases);
  ...
  // Save data to localStorage whenever testCases or saveFolder changes (only after initialization)
  useEffect(() => {
    if (!runId || !isInitialized) return;

    console.log(`Saving test documentor data for ${runId}:`, testCases);
  ...
  }, [testCases, saveFolder, runId, isInitialized]);
              ...
              prev.map((test) =>
                test.id === testId
                  ? {
                      ...
                      hasImageInClipboard: false, // Reset clipboard state after saving
                      imagePasteTimestamp: undefined, // Clear timestamp after saving
                    }
                  : test
              )
            );
  ...
  const handleImagePaste = (testId: string) => {
    // Create timestamp when image is pasted
    const now = new Date();
  ...
  const handleChooseDestination = async () => {
    ...
    // Fallback: Show instructions since we can't actually control download location
    alert(
      "Due to browser security restrictions, files will be downloaded to your default Downloads folder.\n\n" +
        "To organize your screenshots:\n" +
        "1. Files will be saved to Downloads folder\n" +
        "2. Create your desired folder structure\n" +
        "3. Move the downloaded images to your preferred location\n\n" +
        "Tip: You can create a folder and drag the downloaded files there after each screenshot."
    );
    setSaveFolder("Downloads (Browser Default)");
  };
              ...
              <section className="glass-card">
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-4">
                    ...
                    >
                      {saveFolder ? "Change Destination" : "Choose Destination"}
                    </button>
                    {saveFolder && (
...
function TestCaseTile({
  ...
  const handleKeyPress = async (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && !isProcessing && testCase.hasImageInClipboard) {
      ...
      <div className="grid grid-cols-12 gap-4 items-center">
        ...
        <div className="col-span-4">
          <div className="flex space-x-2">
            <div className="flex-1">
              <input
                ...
                onPaste={handlePaste}
                placeholder="Modifier text (16 chars max)"
          ...
          <div className="text-xs mt-1">
            {testCase.hasImageInClipboard && testCase.imagePasteTimestamp ? (
              <span>
                <span className="font-semibold text-green-600">
                  Image pasted
                </span>
                <span className="text-black">, click </span>
                <span className="font-bold text-black">Enter</span>
                <span className="text-black"> to save it. </span>
